...
<body class="bg-green-100">
  ...
  <main class="max-w-7xl mx-auto px-4 my-10">
    ...
    FROM order_items 
    WHERE farmer_id = '$farmer_user_id' AND delivery_status = 'Shipped'
";
...
WHERE oi.farmer_id = '$farmer_user_id' AND oi.delivery_status != 'Shipped'
";
        ...
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-100">
            <?php foreach ($orders_by_buyer as $buyer) {
              ...
              $has_pending = false;
              $first_order_item_id = $all_items[0]['order_item_id']; // For button
              ?>
                ...
                <td class="px-4 py-3">
                  <?php if ($has_pending) { ?>
                    <form method="post" action="mark_shipped_item.php">
                      <input type="hidden" name="order_item_id_farmer" value="<?= $first_order_item_id; ?>">
