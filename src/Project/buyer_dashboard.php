...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delivered_order_id'])) {
    $delivered_id = $_POST['delivered_order_id'];

    include("config1.php"); // if not already included above

    mysqli_query($conn, "DELETE FROM order_items WHERE order_id = $delivered_id AND delivery_status = 'Shipped'");
    
    header("Location: buyer_dashboard.php?msg=Order marked as delivered&type=success");
    exit;
}
...
<body>
    <div>
<header class="bg-white shadow-md p-4 flex justify-between items-center border-b-4 border-green-500 fixed top-0 left-0 w-full z-50">
  ...
  <nav>
    ...
    <ul id="nav-links" class="hidden flex-col md:flex md:flex-row md:space-x-6 space-y-3 text-lg mt-4 md:mt-0 bg-white md:bg-transparent absolute md:static top-16 left-0 w-full md:w-auto p-4 md:p-0 shadow md:shadow-none z-40">
      ...
      <li><a href="logout.php" class="block py-2 px-4 bg-yellow-500 text-black font-bold rounded-lg transition-transform transform hover:scale-110 hover:bg-yellow-600">Logout</a></li>
      <li><a href="index.php" class="block py-2 px-4 bg-yellow-500 text-black font-bold rounded-lg transition-transform transform hover:scale-110 hover:bg-yellow-600">Home</a></li>
    </ul>
  </nav>
</header>
...
<script>
  function toggleMenu() {
    ...
    <div class="">
        <?php if ($showWelcome) { ?>
            <div class="bg-green-100 text-green-700 text-center font-bold p-2 rounded-lg mb-4 mt-10" id="welcomeMessage">
        ...
        <div class="bg-white p-6 rounded-lg shadow mb-6 text-center border-b-4 border-green-500 mt-20" id="profile">
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6 border border-gray-300">
            ...
            <?php if ($buyer) { ?>
                <div class="border p-4 rounded-lg mb-4">
                    ...
                    <p><strong>Address:</strong> <?= $buyer['address'] ?>, <?= $buyer['city'] ?>, <?= $buyer['state'] ?>,
                        <?= $buyer['zip_code'] ?>, <?= $buyer['country'] ?></p>
                    <div class="mt-4">
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6 border border-gray-300">
            ...
            <form class="grid grid-cols-1 md:grid-cols-2 gap-4" method="POST">
                <input type="text" name="name" placeholder="Name" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="email" name="email" placeholder="Email" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="tel" name="phone_no" placeholder="Phone Number" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="text" name="address" placeholder="Address" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="text" name="city" placeholder="City" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="text" name="state" placeholder="State" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="text" name="zip_code" placeholder="Zip Code" class="p-3 border rounded-lg focus:outline-green-500" required>
                <input type="text" name="country" placeholder="Country" class="p-3 border rounded-lg focus:outline-green-500" required>
                <button type="submit" name="save"
                    class="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 col-span-1 md:col-span-2">Save Changes</button>
            </form>
        </div>
    </div>
    <div class="">
                ...
                <tbody>
                    <?php while ($row = $orders->fetch_assoc()) { ?>
                        <tr class="text-center">
                            ...
                            <td class="border border-gray-300 p-2">₹<?= htmlspecialchars($row['total_amount']) ?></td>
        ...
        <?php foreach ($orders as $order_id => $order): ?>
            <div class="bg-white shadow-lg rounded-2xl p-6 border border-green-300">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="text-sm font-medium <?= $order['delivery_status'] === 'Shipped' ? 'text-blue-600' : 'text-yellow-500' ?>">
                        Status: <?= $order['delivery_status'] ?>
                    </span>
                </div>

                <p class="text-sm text-gray-700 mb-2"><strong>Delivery Address:</strong><br><?= nl2br($order['address']) ?></p>
                ...
                <?php if ($order['delivery_status'] === 'Shipped'): ?>
                    <form method="post" class="mt-4 text-right">
                        ...
                        <button type="submit" class="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-medium">
                            Mark as Delivered
                        </button>
                    </form>
                <?php endif; ?>
...
<div class="mt-24 p-4 bg-white shadow rounded-lg overflow-x-auto" id="rates">
  ...
  <!-- 📊 Display Table -->
  <table class="min-w-full text-sm border border-gray-300 bg-white shadow rounded divide-y divide-gray-200">
    ...
    <tbody class="bg-white text-gray-800">
      <?php while($row = mysqli_fetch_assoc($result)) { ?>
        <tr class="hover:bg-gray-50">
          ...
          <td class="px-6 py-4 text-sm text-gray-500"><?= date('d M Y, h:i A', strtotime($row['last_updated'])) ?></td>
        </tr>
      <?php } ?>
    </tbody>
  </table>
...
<section class="bg-green-50 text-gray-800 py-12 px-4 md:px-16 mt-24 rounded-lg shadow-md" id="help">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border border-green-100">
      ...
      <p class="text-sm mb-4">If you have any issues with your orders, profile, or payments, feel free to reach out.</p>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border border-green-100">
      ...
      <ul class="list-disc list-inside text-sm space-y-2">
        <li><strong>How can I track my order?</strong><br> Go to your <a href="order_management.php" class="text-green-600 underline">Order Management</a> section.</li>
        <li><strong>How to update my profile?</strong><br> Use the <a href="profile_management.php" class="text-green-600 underline">Profile Management</a> page to edit your details.</li>
        <li><strong>Need help with payments?</strong><br> Reach out to our support team directly via email.</li>
      </ul>
    </div>
  </div>
</section>
